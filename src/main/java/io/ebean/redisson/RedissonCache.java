 ...
 * Feature handling:
 * <ul>
 *   <li><b>maxSecsToLive</b> - authoritative native per-field TTL (set on every write).</li>
 ...
 *       reads as plain {@code HGET} instead of turning every read into a Redis write. When only
 *       {@code maxIdleSecs} is set it becomes the TTL and is slid forward on each read.</li>
 *   <li><b>maxSize</b> - native hashes are not bounded, so size is enforced by a best-effort periodic
...
public class RedissonCache implements ServerCache {
    ...
    /**
     * How often (seconds) the best-effort maxSize trim runs.
     */
     ...
     * format is {@code [marker][8-byte big-endian version][bean]} ({@code VersionGatedCodec}). For each pair
     * we skip the write only when the stored value carries the marker (so it has a comparable version) AND
     * its version is strictly newer than the incoming one (comparing the 8 bytes after the marker
     * lexicographically, which equals numeric order for non-negative versions). A stored value WITHOUT the
     * marker is a legacy/un-prefixed entry with no comparable version, so it is always overwritten - this is
     * what keeps the gate backward compatible (it neither misreads legacy bytes as a huge version nor leaves
     * the stale legacy entry pinned). Then apply the per-field TTL. One round trip.
     */
    ...
    /**
     * TTL applied on every write, or {@code null} for entries that never expire.
     */
    private final Duration writeTtl;

    /**
     * When true, the TTL is slid forward on each read to honour maxIdle. Only enabled when there
     * is no hard maxSecsToLive cap (otherwise reads would become writes for no correctness benefit).
     */
    ...
    RedissonCache(RedissonClient redissonClient, ServerCacheConfig config, Codec codec, BackgroundExecutor executor, boolean versionGated) {
        ...
        if (maxSecsToLive > 0) {
            // hard TTL is authoritative; do not slide on read
            this.writeTtl = Duration.ofSeconds(maxSecsToLive);
            this.slideIdle = false;
            this.idleTtl = null;
        } else if (maxIdleSecs > 0) {
            // no hard cap: maxIdle is the TTL and is slid forward on each read
    ...
    /**
     * A read against Redis failed. Treat it as a cache miss and degrade to the database.
     */
    ...
    private void errorOnWrite(Exception e) {
        // writes here include invalidation (remove/clear); surface the failure so a missed
        // invalidation is not silently swallowed (which could leave stale entries cluster-wide).
    ...
    /**
     * Slide the idle TTL forward in the background so it never adds latency to the read path.
     */
    ...
    /**
     * Version-gated put: never overwrites a strictly newer cached version. Done atomically in a single
     * Lua compare-and-set (one Redis round trip, no lock), comparing the version that {@code VersionGatedCodec}
     * stores after its marker. A legacy entry without the marker has no comparable version and is overwritten.
     */
    ...
    /**
     * Encode a value to the raw bytes Redis stores (version prefix + bean), via the configured codec.
     */
    ...
    /**
     * Best-effort bounding of the remote region to maxSize. Native hashes have no LFU/LRU eviction,
     * so this periodically removes the surplus once the region exceeds maxSize. Victims are taken in
     * Redis scan order (approximate) - TTL/maxIdle already evict cold entries, this is only a backstop
     * against unbounded growth. Runs off the hot path on the background executor.
     */
    void trimCache() {
        try {
